...
def parking_lot_rate(data_dir, output_dir, data):
    df = pd.read_csv(os.path.join(data_dir, "Data/주차장_확보율.csv"))
    col = df.columns
    ...
    df.to_csv(os.path.join(output_dir, "Data/주차장_확보율.csv"), index=False)

def public_parking(data_dir, output_dir, data):
    df = pd.read_csv(os.path.join(data_dir, "Data/서울시 공영주차장 안내 정보.csv"), encoding='cp949')
...
def gangseo_parking(data_dir, output_dir):
    ...
    df_seoul = pd.read_csv(os.path.join(data_dir, "Data/서울시 공영주차장 안내 정보.csv"), encoding='cp949')
    ...
    df_gangseo = pd.read_csv(os.path.join(data_dir, "Data/강서구시설관리공단_공영주차장_현황.csv"), encoding='cp949')
