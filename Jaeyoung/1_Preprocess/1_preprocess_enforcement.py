...
def merge_enforcement(data_dir, output_dir):
    ...
    df = pd.read_csv(os.path.join(data_dir, "불법_주정차_단속_실적.csv"), header=1) # 2017~2022 서울시 불법 주정차 단속 실적
    ...
    df.to_csv(os.path.join(output_dir, "불법_주정차_단속_실적.csv"), index=False)
